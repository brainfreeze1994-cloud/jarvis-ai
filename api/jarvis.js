// jarvis.js — Multi-provider LLM with Groq (primary) + Cloudflare (fallback)
// Groq free tier: 14,400 req/day (llama-3.1-8b) or 1,000 req/day (llama-3.3-70b)
// Cloudflare: 10,000 neurons/day (backup)
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── IMAGE ANALYSIS ────────────────────────────────────────────────────────
    if (imageBase64 && ACCOUNT_ID && API_TOKEN) {
...
async function callLLM(groqKey, accountId, apiToken, messages) {
  ...
  // 1. GROQ — primary (free, fast, generous limits, resets daily)
  if (groqKey) {
    var groqModels = [
    ...
    for (var i = 0; i < groqModels.length; i++) {
      try {
        ...
        var gErr = (gData.error && gData.error.message) || ('Groq ' + groqModels[i] + ' status ' + gRes.status);
        errors.push(gErr);
  ...
  // 2. CLOUDFLARE — fallback (10,000 neurons/day)
  if (accountId && apiToken) {
    ...
    for (var j = 0; j < cfModels.length; j++) {
      try {
        ...
        var cfErr = (cfData.errors && cfData.errors[0] && cfData.errors[0].message) || ('CF ' + cfModels[j] + ' status ' + cfRes.status);
        errors.push(cfErr);
      } catch (e) {
        errors.push('CF network: ' + e.message);
      }
    }
  }
