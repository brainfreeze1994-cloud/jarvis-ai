module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const runMatch = !codeBlockMatch && lastMsg.match(/^(?:run|execute|compile)\s+(?:this\s+)?(?:(\w+)\s+)?(?:code[:\s])?([\s\S]+)/i);
      ...
      try {
        ...
        if (wRes.ok) {
          ...
          const forecastLines = w.weather.slice(0, 3).map((day, i) => {
            ...
            return `**${days[i]} (${date}):** ${minC}°C – ${maxC}°C, ${dayDesc}, 🌧 ${rain}% rain`;
          }).join('\n');
...
function buildSystemPrompt(now) {
  return `You are J.A.R.V.I.S (Just A Rather Very Intelligent System), an advanced AI assistant like ChatGPT or Claude — with a slightly formal tone and dry wit. Occasionally address the user as "sir".

Current date and time: ${now} (Dubai, UTC+4)

You can:
- Answer any question in full depth with complete explanations
...
- Search the web, read URLs, execute code, and generate images when asked
...
- State clearly when you're uncertain rather than guessing
